...
contract CropInsurance is Ownable {
    ...
    uint256 public constant RAINFALL_THRESHOLD = 500;//降雨量阈值：低于500就赔钱
    uint256 public constant INSURANCE_PREMIUM_USD = 10;//保费10美元
    uint public constant INSURANCE_PAYOUT_USD = 50;//赔款50美元
    
    mapping(address => bool) public hasInsurance;//农民是否买了保险：true=买了
    mapping(address => uint256) public lastClaimTimestamp;//农民上次赔钱的时间，防止一天内反复索赔
    ...
    function purchaseInsurance()external payable {
        uint256 ethPrice = getEthPrice();//查eth兑美元的价格
        uint256 premiumInEth = (INSURANCE_PREMIUM_USD * 1e18) / ethPrice;//算10美元等于多少ETH
    ...
    function checkRainfallAndClaim() external {
        ...
        ) = weatherOracle.latestRoundData();//从天气预报机提取最新的降雨数据
        ...
        if (currentRainfall < RAINFALL_THRESHOLD){
            ...
            emit ClaimPaid(msg.sender, payoutInEth);//记录赔钱记录
        }
    }
